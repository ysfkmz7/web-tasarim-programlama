import { useState } from "react";
import type { ReactNode } from "react";

/* ─── Tip Tanımları ─── */
type AlertVariant = "info" | "success" | "warning" | "error";

interface AlertProps {
  variant?: AlertVariant;
  title?: string;
  children: ReactNode;
  dismissible?: boolean;
  className?: string;
}

/* ─── Variant → Stil Haritası ─── */
const variantConfig: Record<
  AlertVariant,
  {
    container: string;
    icon: ReactNode;
    titleColor: string;
    textColor: string;
  }
> = {
  info: {
    container:
      "bg-info-bg border border-info-border text-info",
    icon: (
      <svg className="w-5 h-5 shrink-0" fill="currentColor" viewBox="0 0 20 20">
        <path
          fillRule="evenodd"
          d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z"
          clipRule="evenodd"
        />
      </svg>
    ),
    titleColor: "text-info",
    textColor:  "text-info/80",
  },
  success: {
    container:
      "bg-success-bg border border-success-border text-success",
    icon: (
      <svg className="w-5 h-5 shrink-0" fill="currentColor" viewBox="0 0 20 20">
        <path
          fillRule="evenodd"
          d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
          clipRule="evenodd"
        />
      </svg>
    ),
    titleColor: "text-success",
    textColor:  "text-success/80",
  },
  warning: {
    container:
      "bg-warning-bg border border-warning-border text-warning",
    icon: (
      <svg className="w-5 h-5 shrink-0" fill="currentColor" viewBox="0 0 20 20">
        <path
          fillRule="evenodd"
          d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z"
          clipRule="evenodd"
        />
      </svg>
    ),
    titleColor: "text-warning",
    textColor:  "text-warning/80",
  },
  error: {
    container:
      "bg-error-bg border border-error-border text-error",
    icon: (
      <svg className="w-5 h-5 shrink-0" fill="currentColor" viewBox="0 0 20 20">
        <path
          fillRule="evenodd"
          d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z"
          clipRule="evenodd"
        />
      </svg>
    ),
    titleColor: "text-error",
    textColor:  "text-error/80",
  },
};

/* ═══════════════════════════════════════════════════════════════
   Alert Bileşeni
   ═══════════════════════════════════════════════════════════════ */
export default function Alert({
  variant = "info",
  title,
  children,
  dismissible = false,
  className = "",
}: AlertProps) {
  const [visible, setVisible] = useState(true);

  if (!visible) return null;

  const config = variantConfig[variant];

  return (
    <div
      role="alert"
      className={[
        "flex items-start gap-3 rounded-xl px-4 py-3.5",
        "transition-all duration-200",
        config.container,
        className,
      ]
        .filter(Boolean)
        .join(" ")}
    >
      {/* İkon */}
      <span className="mt-0.5">{config.icon}</span>

      {/* İçerik */}
      <div className="flex-1 min-w-0">
        {title && (
          <p className={`font-semibold text-sm mb-0.5 ${config.titleColor}`}>
            {title}
          </p>
        )}
        <div className={`text-sm leading-relaxed ${config.textColor}`}>
          {children}
        </div>
      </div>

      {/* Kapat Butonu */}
      {dismissible && (
        <button
          type="button"
          aria-label="Kapat"
          onClick={() => setVisible(false)}
          className="shrink-0 mt-0.5 p-0.5 rounded-md opacity-70 hover:opacity-100 transition-opacity duration-150 focus:outline-none focus:ring-2 focus:ring-current/50"
        >
          <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
            <path
              fillRule="evenodd"
              d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
              clipRule="evenodd"
            />
          </svg>
        </button>
      )}
    </div>
  );
}
